        ...
        try:
            with winreg.OpenKey(hkey, subkey, 0, winreg.KEY_READ) as key:
                ...
                for path_dir in path_val.split(os.path.pathsep):
                    ...
                    if (
                        path_dir_expanded
                        and path_dir_expanded not in os.environ["PATH"]
                    ):
...
class AppConfig:
    ...
    @classmethod
    def get_elan_dir(cls):
        return os.path.join(cls.SANDBOX_DIR, "processed-elan-files")

    @classmethod
    def get_wav_dir(cls):
        return os.path.join(cls.SANDBOX_DIR, "wav")

    @classmethod
    def get_metadata_csv(cls):
        return os.path.join(cls.SANDBOX_DIR, "wav-metadata.csv")

    @classmethod
    def get_inf_dir(cls):
        return os.path.join(cls.SANDBOX_DIR, "audiofiles-to-transcribe")

    @classmethod
    def get_model_dir(cls):
        return os.path.join(cls.SANDBOX_DIR, "wav2vec2-model")
...
def get_julie_segments(req: JulieVADRequest):
    try:
        ...
        if (
            cache_key not in audio_profile_cache
            or audio_profile_cache[cache_key]["mtime"] != mtime
        ):
        ...
        for seg in segments:
            sec_segments.append(
                {
                    "start": round(seg["start"] / 1000.0, 3),
                    "end": round(seg["end"] / 1000.0, 3),
                }
            )
...
def process_file_preview(
    ...
    min_silence = (
        custom_setting.min_silence_len if custom_setting else default_min_silence
    )
    keep_silence = (
        custom_setting.keep_silence if custom_setting else default_keep_silence
    )
    ...
    coverage_percent = (
        round((result_duration / total_duration * 100.0), 1)
        if total_duration > 0
        else 0.0
    )
    overlap_duration = round(overlap_ms / 1000.0, 2)
    overlap_percent = (
        round((overlap_duration / total_duration * 100.0), 1)
        if total_duration > 0
        else 0.0
    )
    ...
    if not all_lengths:
        ...
        if max_val > min_val:
            ...
            for i in range(bins):
                ...
                histogram.append({"start": start, "end": end, "count": bin_counts[i]})
        else:
...
def preview_segments(req: PreviewSegmentRequest):
    try:
        ...
        if not os.path.exists(target_full_path):
            raise HTTPException(
                status_code=404, detail=f"Path not found: {req.target_path}"
            )
        ...
        overall_coverage_percent = (
            round((total_result_duration / total_batch_duration * 100.0), 1)
        ...
        for thresh in thresholds:
            for min_sil in min_silence_lens:
                for keep_sil in keep_silences:
                    ...
                    overlap_percent = (
                        (overlap_ms / total_len * 100.0) if total_len > 0 else 0.0
                    )
    ...
    try:
        ...
        with open(manifest_path, mode="a", newline="", encoding="utf-8") as csvfile:
            ...
            if not manifest_exists:
                writer.writerow(
                    ["original_filename", "segmented_filepath", "start_ms", "end_ms"]
                )
            ...
            for rel_path in files_to_process:
                ...
                if (
                    cache_key in audio_profile_cache
                    and audio_profile_cache[cache_key]["mtime"] == mtime
                ):
                    cached = audio_profile_cache[cache_key]
        ...
        return {
            "message": f"Processed {segmented_count} files into {total_chunks} segments.",
            "output_dir": out_dir,
        }
    except Exception as e:
        import traceback
...
def run_batch_inference(req: BatchInferenceRequest):
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_batch_dir:
            ...
            if not mapping:
                raise Exception(
                    "No supported audio files (.wav, .mp3, etc.) found in selected folders."
                )

            temp_csv = os.path.join(temp_batch_dir, "inference_temp.csv")
            python_exe = os.path.join(
                AppConfig.SANDBOX_DIR, "venv", "Scripts", "python.exe"
            )
            ...
            if req.checkpoint:
                cmd.extend(
                    ["--checkpoint", req.checkpoint, "--processor", req.checkpoint]
                )
            print(f"Running batched inference on {len(mapping)} files...", flush=True)
            result = subprocess.run(cmd, cwd=AppConfig.SANDBOX_DIR)
            if result.returncode != 0:
                raise Exception(
                    f"Batch inference failed with exit code {result.returncode}. Check terminal for details."
                )
            ...
            df["file_path"] = df["file_path"].apply(
                lambda x: mapping[os.path.basename(x)]
            )
        ...
        return {
            "message": f"Batch inference complete for {len(mapping)} files across {len(req.target_folders)} folders.",
            "csv_path": output_csv,
        }
    except Exception as e:
        import traceback
...
def get_files():
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        dirs[:] = [
            d
            for d in dirs
            if d not in ["venv", "node_modules", ".git", "__pycache__", "training_data"]
...
def get_inference_files():
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        dirs[:] = [
            d
            for d in dirs
            if d not in ["venv", "node_modules", ".git", "__pycache__"]
            and not d.startswith("logs-")
        ]
...
def addZeros(num, max_num):
    return str(num).zfill(countDigits(max_num))
...
def process_elan(req: ProcessElanRequest):
    try:
        ...
        for i, (start, end) in enumerate(zip(timeStart, timeEnd)):
            tempName1 = (
                f"{speaker_code}-{audio_prefix}-{addZeros(i+1, len(timeStart))}.wav"
            )
            tempName2 = (
                f"{speaker_code}-{audio_prefix}-{addZeros(i+2, len(timeStart))}.wav"
            )
        ...
        outFileName = os.path.join(
            AppConfig.get_wav_dir(), f"{speaker_code}-{audio_prefix}-%0{zeros}d.wav"
        )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error reading metadata CSV: {str(e)}"
            )
        ...
        for i in range(len(timeStart)):
            ...
            new_rows.append(
                {
                    "wav_filename": fname,
                    "sandbox": os.path.basename(AppConfig.SANDBOX_DIR).replace(
                        "sandbox-", ""
                    ),
                    "date": todaysDate,
...
def generate_splits(req: GenerateSplitsRequest):
    ...
    if not os.path.exists(AppConfig.get_metadata_csv()):
        raise HTTPException(
            status_code=404, detail="Metadata CSV not found. Process ELAN first."
        )
    ...
    if "duration_seconds" in df.columns:
        df["duration_seconds"] = (
            df["duration_seconds"].astype(str).str.replace(",", ".").astype(float)
        )
    ...
    paths = [
        os.path.join(AppConfig.get_wav_dir(), row["wav_filename"]).replace("\\", "/")
...
def train_model(req: TrainRequest):
    ...
    try:
        ...
        return {
            "message": "Training started in the background. Check terminal for logs."
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        ...
        with open(csv_file, mode="r", encoding="utf-8") as f:
            ...
            txt_col = (
                ...
                if "greedy_transcription" in reader.fieldnames
                else "sentence" if "sentence" in reader.fieldnames else None
            )
                    ...
                    try:
                        ...
                        if len(word_confs) == 1 and len(greedy_words) > 1:
                            ...
                            for tw in greedy_words:
                                ...
                                if w_chars:
                                    ...
                                    if "start_time" in w_chars[0]:
                                        new_word["start_time"] = w_chars[0][
                                            "start_time"
                                        ]
                                        new_word["end_time"] = round(
                                            w_chars[-1].get("start_time", 0) + 0.02, 3
                                        )
                            ...
                            if char_idx < len(all_chars):
                                ...
                                avg_c = (
                                    float(
                                        np.mean([c.get("confidence", 0.0) for c in rem])
                                    )
                                    if rem
                                    else 0.0
                                )
...
def save_labels(req: SaveLabelsRequest):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Successfully saved {len(req.labels)} labels",
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
