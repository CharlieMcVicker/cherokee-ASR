...
def yield_hf_revisions(repo_id, revisions, token=None):
    ...
    for friendly_name, rev_hash in revisions:
        ...
        try:
            processor = Wav2Vec2Processor.from_pretrained(repo_id, token=token, revision=rev_hash)
        except Exception:
            processor = fallback_processor
        yield f"{friendly_name} ({rev_hash[:7]})", model, processor, f"hf://{repo_id}@{rev_hash}"
        del model

def yield_single_checkpoint(checkpoint_path, processor_path=None, revision=None, token=None):
    """
    Yields a single local or HF checkpoint/model.
    """
    proc_path = processor_path or checkpoint_path
    processor = Wav2Vec2Processor.from_pretrained(proc_path, token=token, revision=revision)
    model = Wav2Vec2ForCTC.from_pretrained(checkpoint_path, token=token, revision=revision)
    yield "checkpoint", model, processor, checkpoint_path
    del model

def run_evaluation(model_generator, test_ds_prepared, data_collator=None, batch_size=16):
    """
    ...
    for label, model, processor, path in model_generator:
        ...
        if data_collator is None:
            class SimpleDataCollator:
                ...
                def __call__(self, features):
                    ...
                    return self.proc.pad(input_features, padding=True, return_tensors="pt")
        ...
        for batch in tqdm(test_loader, desc=f"Inference ({label})"):
            ...
            if attention_mask is not None:
                ...
                output_lengths = model._get_feat_extract_output_lengths(input_lengths).cpu().numpy()
            else:
        ...
        ranking.append({
            ...
            "agg_wer_greedy_masked": jiwer_wer([safe(strip_length(g)) for g in golds], [safe(strip_length(h)) for h in greedies]),
            "agg_cer_greedy_masked": jiwer_cer([safe(strip_length(g)) for g in golds], [safe(strip_length(h)) for h in greedies]),
            "agg_wer_tone_masked": jiwer_wer([safe(strip_tones(g)) for g in golds], [safe(strip_tones(h)) for h in greedies]),
            "agg_cer_tone_masked": jiwer_cer([safe(strip_tones(g)) for g in golds], [safe(strip_tones(h)) for h in greedies]),
            "agg_wer_both_masked": jiwer_wer([safe(strip_both(g)) for g in golds], [safe(strip_both(h)) for h in greedies]),
            "agg_cer_both_masked": jiwer_cer([safe(strip_both(g)) for g in golds], [safe(strip_both(h)) for h in greedies]),
        })
