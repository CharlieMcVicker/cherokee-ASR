# -*- coding: utf-8 -*-
"""
digohwelisgi.cherokee.codeswitching.codeswitched_preparer module.
...
from digohwelisgi.cherokee.orthography import Orthography, convert_orthography
...
def prepare_code_switched_token(
    ...
    if projector is not None:
        ...
        from digohwelisgi.cherokee.codeswitching.projector import (
...
class CodeSwitchedPreparer:
    ...
    def prepare_line(
        self, text: str, strip_speaker: bool = False
    ) -> CodeSwitchedLineResult:
        return create_groundtruth_for_code_switched_syllabary(
            text=text,
            projector=self.projector,
            strip_speaker=strip_speaker,
            contextual_preaspiration=self.contextual_preaspiration,
        )

    def prepare_token(self, token: str) -> CodeSwitchedToken:
        return prepare_code_switched_token(
            token=token,
            projector=self.projector,
            contextual_preaspiration=self.contextual_preaspiration,
        )
