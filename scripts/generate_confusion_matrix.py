...
def main():
    ...
    for name, df in datasets.items():
        ...
        # Resolve paths
        df['path'] = df['path'].apply(lambda p: p if os.path.exists(p) else os.path.join("training_data/processed/sentence_audio", os.path.basename(p)))
        # Verify files exist
        ...
        ds_prepared = ds.map(prepare_batch, remove_columns=[c for c in ds.column_names if c != "sentence"], num_proc=1)
        ...
        plot_and_save_matrix(matrix, f"Character Confusion Matrix - {name.capitalize()} Data", img_path)
