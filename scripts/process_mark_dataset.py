...
3. Generates cherokee_new_testament/train_csvs/mark.csv with columns: path, sentence.
4. Computes audio duration metrics (min, max, median, total seconds).
"""
...
def main():
    ...
    for ch in range(1, 17):
        ch_str = f"{ch:02d}"
        audio_path = AUDIO_SRC_DIR / f"mark_{ch_str}.mp3"
        transcript_path = TRANSCRIPTS_DIR / f"mark_{ch_str}.json"
        ...
        print(f"\n--- Aligning Mark Chapter {ch_str} ---")
        out_praat_ch = PRAAT_OUT_DIR / f"mark_{ch_str}"
        ...
        for v_idx, v in enumerate(alignment.verses, 1):
            ...
            wav_filename = f"mark_{ch_str}_{verse_num_str}.wav"
    ...
    csv_out_path = TRAIN_CSVS_DIR / "mark.csv"
    ...
    if durations_sec:
        ...
        print("\n=== Audio Statistics for Book of Mark ===")
        ...
        print(
            f"Total audio duration : {total_sec:.2f} seconds ({total_sec/60:.2f} minutes)"
        )
