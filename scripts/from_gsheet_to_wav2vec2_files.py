...
def list_installations():
    ...
    return sorted(
        d
        for d in os.listdir(MYDRIVE_ROOT)
...
def _drive_folder_id(service, installation):
    q = (
        f"name = '{_drive_escape(installation)}' and "
...
def find_sheet_url_via_api(sandbox, log=print):
    ...
    try:
        ...
        if installation and not folder_id:
            log(
                f"   couldn't locate the installation folder "
                f"'{installation}' in your Drive"
            )
            return ""
        name = SHEET_NAME_PREFIX + sandbox
        q = (
            f"name = '{_drive_escape(name)}' and "
        ...
        if files:
            if len(files) > 1:
                log(
                    f"   note: {len(files)} sheets named '{name}' in "
...
install_dd = widgets.Dropdown(
    ...
    value=(
        ...
        if DEFAULT_INSTALLATION_FOLDER in _inst_options
...
sandbox_dd = widgets.Dropdown(
    description="Sandbox (destinationSandbox):",
    options=[],
    style=label_style,
    layout=row_layout,
)
...
url_tb = widgets.Text(
    description="Sheet URL (urlSandbox):",
    placeholder="auto-detected from the sheet in this folder",
    style=label_style,
    layout=widgets.Layout(width="640px"),
)
sheet_btn = widgets.Button(
    description="🔗 Find Sheet URL",
    tooltip="Look the sheet up in Google Drive " "(asks to authorize the first time)",
    layout=widgets.Layout(width="180px"),
)
path_lbl = widgets.HTML()

# Model / data options.
software_dd = widgets.Dropdown(
...
prefix_tb = widgets.Text(
    description="File prefix (filePrefix):",
    value="cim-wav2vec2",
    style=label_style,
    layout=row_layout,
)
maxdur_tb = widgets.BoundedIntText(
    description="Max WAV duration (s):",
    value=15,
    min=1,
    max=600,
    style=label_style,
...
cs_cb = widgets.Checkbox(
    value=False, description="Use code-switched data", indent=False
)
dd_cb = widgets.Checkbox(value=False, description="Use doubtful data", indent=False)

# Train / valid / test split.
train_pct = widgets.BoundedIntText(
    description="Train %:",
    value=80,
    min=0,
    max=100,
...
valid_pct = widgets.BoundedIntText(
...
test_pct = widgets.BoundedIntText(
    description="Test %:",
    value=10,
    min=0,
    max=100,
...
def update_url_and_path(*_):
    ...
    if path and os.path.isdir(path):
        path_lbl.value = (
            f"<span style='color:#1a7f37'>🎧 pathAudioFilesInTraining = "
            f"<code>{path}</code></span>"
        )
    elif path:
        path_lbl.value = (
            f"<span style='color:#c0392b'>🎧 pathAudioFilesInTraining = "
...
def populate_sandboxes(*_):
    sbs = list_sandboxes()
    default_sb = "sandbox-user" if "sandbox-user" in sbs else (sbs[0] if sbs else None)
...
def on_pct_change(*_):
    ...
    if total == 100:
        ...
        pct_lbl.value = (
            f"<span style='color:#c0392b'>split adds up to {total}% "
            f"— must be 100%</span>"
        )
...
def on_find_sheet(_):
    with status_out:
        ...
        if url:
            ...
            return
        print(
            f"Looking up '{SHEET_NAME_PREFIX}{sb}' in Google Drive "
...
def on_confirm(_):
    ...
    with status_out:
        ...
        if not os.path.isdir(MYDRIVE_ROOT):
            problems.append(
                f"MyDrive not found at {MYDRIVE_ROOT} " f"(is Google Drive mounted?)."
            )
        ...
        if not urlSandbox:
            problems.append(
                "Sheet URL is empty — click 🔗 Find Sheet URL " "or paste it in."
            )
...
savePath = (
    "/content/drive/MyDrive/" + installationFolder + "/" + destinationSandbox + "/"
)  # This is where the metadata files will be stored
...
metadata["duration_seconds"] = pd.to_numeric(
    metadata["duration_seconds"].str.replace(",", ".")
)
...
samplesUpToTrainPartition = int(
    round(len(listOfNumbers) * (percentageTrainSet / 100), 0)
)
samplesUpToTestPartition = int(
    round(len(listOfNumbers) * ((percentageTrainSet + percentageValidSet) / 100), 0)
)
...
def writeCSVFile(header, software, inPath, inSentences, inSizes, inFilename):
    ...
    if software == "wav2vec2":
        for i in range(0, len(inPath)):
            output = output + inPath[i] + "," + inSentences[i] + "\n"
    elif software == "ds":
        for i in range(0, len(inPath)):
            output = output + inPath[i] + "," + inSizes[i] + "," + inSentences[i] + "\n"
    output = output[:-1]
    f = open(inFilename, "w")
    f.write(output)
    f.close()
